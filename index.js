...
export const getCurrentInputValue = () => {
  const values = {
    filterType: localStorage.getItem("filterBy")
...
const storeFilterSelected = (event) => {
  if (event.target.value !== "select") select.classList.remove("error");
  localStorage.setItem("filterBy", event.target.value);
};

const storeInputValue = (event) => {
  if (event.target.value !== "select") input.classList.remove("error");
